...
if($conn){
    ...
    if(isset($_POST['submitForm'])){
        ...
        if(empty($leaderName) || empty($instNameLeader) || empty($fullNameMember1)|| empty($instNameMember1) || empty($emailLeader) || empty($contactLeader) ){
           ...
           $fake = mysqli_fetch_assoc($result);
           if ($fake) { // if user exists
                  if ($fake['emailid'] === $emailLeader) {
                    array_push($errors, "Already exists");
                  }
            }
           if (count($errors) == 0) {

                $sql="insert into aavegame (fullname,institution, emailid, contactnum1, fullNameMember2, contactnum2, fullNameMember3, contactnum3) values('$leaderName','$instNameLeader','$emailLeader','$contactLeader','$fullNameMember1','$instNameMember1','$fullNameMember2','$instNameMember2')";
